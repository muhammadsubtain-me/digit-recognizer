...
class DigitClassifier:
    ...
    def load_dataset(self, folder, mode):
        ...
        if mode == "train":
            ...
            self.test_images = np.array(images_list)
            self.test_labels = np.array(labels_list)
    ...
    def test_model(self, folder):
        # Step a: Load the test dataset (same preprocessing as load_dataset)
        self.load_dataset(folder, "test")

        # Step b: Predict labels using the loaded model
        predicted_labels = self.model.predict(self.test_images)

        # Step c: Compute accuracy score
        acc = accuracy_score(self.test_labels, predicted_labels)

        # Step d: Print actual and predicted labels for visual comparison
        print("\n===== Testing Phase Results =====")
        print(f"Actual labels   : {self.test_labels.tolist()}")
